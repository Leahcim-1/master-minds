...
\end{minted}
Each turn, a guess is played by the algorithm (codebreaker) and a response received from the codemaker. If a number of black pegs equal to the number of holes is returned, the code has been found! In some cases, the algorithm will luckily guess the correct solution without having to reduce $|P|$ to $1$. Otherwise, $P$ is filtered using the response and then the universe of possible codes $S$ is searched for the best next guess.
...
% TODO: image of 8765 before parallelization threadscope -- make sure this is "below" as explained in text or update text with figure number
...
% TODO: image from "First parMap attempt"
...
% TODO: line graph of number of cores vs. performance improvement
...
\subsection{Chunks}
Thus, we designed a parallelization strategy which splits the candidate codes into $n$ chunks and then parallelizes computation over those chunks.
% TODO discuss chunking code and algorithm improvement too
% TODO discuss improvement as number of chunks increases
% TODO load balancing
